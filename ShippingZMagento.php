...
define("SHIPPINGZMAGENTO_VERSION","3.0.7.0468");
...
class ShippingZMagento extends ShippingZGenericShoppingCart
{
	...
	function UpdateShippingInfo($OrderNumber,$TrackingNumber='',$ShipDate='',$ShipmentType='',$Notes='',$Carrier='',$Service='',$ShippingCost='')
	{
		...
		if(!is_numeric($OrderNumber)) //check if order number is not integer
		{
	   ...
	   else
	   {
	   		try
			{
				...
				$Service="Shipping Tracking";
				 ...
				 $track = Mage::getModel('sales/order_shipment_track')->setNumber($coreResource->quote($TrackingNumber));
        
				
				$track->setCarrierCode(strtolower($SelectedCarrier));
				$track->setTitle($coreResource->quote($SelectedCarrier." ".$Service));
