...
mst(document).ready(function ($) {
    ...
    $('.wrap_inlay_center').each(function(){
       ...
       $(this).append('<img id="main_image_'+ tab+'" src="' + m + 'media/pdp/images/no_image.jpg" />'); 
    });
    ...
    if ($('#list_color li').length > 0) {
        if ($('#list_color li[tt="' + o + '"]').length > 0) {
            if(first_item!=''){
                ...
                $('#main_image_left').attr("src", m + 'media/pdp/images/' + first_item.attr("rell"));
                $('#main_image_right').attr("src", m + 'media/pdp/images/' + first_item.attr("relr"));
            }
            ...
            $('#canvas_area_front').attr({
                'width': inlay_f[0],
                'height': inlay_f[1]
            });
            var canvas_f = new fabric.Canvas('canvas_area_front', {
                opacity: 1
            });
            //canvas_f.setBackgroundImage(m + 'media/pdp/images/' + first_item.attr("relf"),function(){canvas_f.renderAll()});
            $('#canvas_area_back').attr({
                'width': inlay_b[0],
                'height': inlay_b[1]
            });
            var canvas_b = new fabric.Canvas('canvas_area_back', {
                opacity: 1
            });
            $('#canvas_area_left').attr({
                'width': inlay_l[0],
                'height': inlay_l[1]
            });
            var canvas_l = new fabric.Canvas('canvas_area_left', {
                opacity: 1
            });
            $('#canvas_area_right').attr({
                'width': inlay_r[0],
                'height': inlay_r[1]
            });
            var canvas_r = new fabric.Canvas('canvas_area_right', {
                opacity: 1
            });
            ...
            ////////////////////////No Group////////////////////////////////////
            canvas_f.selection = false;
            canvas_b.selection = false;
            canvas_l.selection = false;
            canvas_r.selection = false;
        }
    };
    ...
    $('.add_artwork > a').on('click',function(){
       $('.tab_content:not(#select_image)').slideUp();
       $('#select_image').slideToggle();
    });
    $('.add_text > a').on('click',function(){
       $('#add_text').slideToggle();
       $('#font_outline_colorpicker').hide();
       $('.tab_content:not(#add_text), .pdp_extra_item').slideUp();
    ...
    $('#rotate-180').click(function () {
        $('#pdp_rotate_item').slideToggle();
        $('.tab_content:not(#pdp_rotate_item)').slideUp();
    });
    $('#pdp_side_items li').click(function(){
        ...
        if($('.product-image.act .wrap_inlay_center').attr("tab")!=tab){
            $('.product-image.act').removeClass("act");
            $('.wrap_inlay_center').parent().hide();
            $('.wrap_inlay_center[tab='+tab+']').parent().addClass("act").show();
    ...
    $('#pdp_edit_text_tab a').click(function(){
       ...
       $(this).addClass('active');
       $('.pdp_edit_text_tab_content').slideUp();
    ...
    $('#pdp_font_size_input').bind("slider:changed", function (event, data) {
        ...
        var cv_act = $('.product-image.act .wrap_inlay_center').attr('tab'),
            canvas;
        switch (cv_act) {
            case 'front' : canvas = canvas_f; break;
            case 'back' : canvas = canvas_b; break;
            case 'left' : canvas = canvas_l; break;
            case 'right' : canvas = canvas_r; break;
        }
    ...
    $('.design_label span').click(function(){
       $('#image_category_list').slideToggle(); 
    });
    $('#image_category_list').hover(function(){
        $(this).stop(true,false).animate({'height': h_cate_list},300); 
    },function(){
       $(this).stop(true,false).animate({'height': '0px'},300,function(){
    ...
    $('#font_outline_color').click(function(){
       $('#font_outline_colorpicker').slideToggle(); 
    });
    ...
    $('#pdp_color_text li a').click(function(){
		...
		if(color==='transparent'){ //resetColor(); 
		}else{
    ...
    $('#icon_list, #lists_img_upload, #photos_album').on('click', 'img', function () {
        ...
        $('#design_control .tab_content').slideUp();
    ...
    var canvasEvents = {
        ...
        objectUnselected: function () {
            //canvasEvents.resetTextForm();
            $('#edit_item_wrap, .pdp_extra_item, .tab_content, #font_outline_colorpicker').slideUp();
            $('#edit_item_wrap .active').removeClass('active');
        },
        clearSelected: function ()  {
            this.objectUnselected();
            canvas_f.deactivateAll().renderAll();
            canvas_b.deactivateAll().renderAll();
            canvas_l.deactivateAll().renderAll();
            canvas_r.deactivateAll().renderAll();
        },
        centerX: function () {
            var cv_act = $('.product-image.act .wrap_inlay_center').attr('tab'),
                canvas;
        ...
        renderall: function(){
            canvas_f.calcOffset().renderAll();
            canvas_b.calcOffset().renderAll();
            canvas_l.calcOffset().renderAll();
            canvas_r.calcOffset().renderAll();
        },
        centerCanvas: function(){
            var w_wrap = $('.product-image.act').width(),
    ...
    $('#edit_item_wrap .item').click(function(){
       ...
       $('.pdp_extra_item:not(#'+tab_act+')').slideUp();
       $('#'+tab_act).slideToggle();
    });
    //////////////////////////////////EDIT FUNCTIONS/////////////////////////////////////////////////////////////////
    ...
    */
    ...
    $('html').on('click', function (e) {
        var target = $('#canvas-container, #select_image, #add_text, #canvas_area, .pdp_extra_item, #pdp_opacity_item, .color_content, .control_tab, .canvas-container, .edit_area , #add_text, .control_area, #edit_item_wrap').has(e.target).length;
        if (target === 0) {
            canvasEvents.clearSelected();
        }
        var cl_target = $('#font_outline_colorpicker, #font_outline_color').has(e.target).length;
        if (cl_target===0){
            $('#font_outline_colorpicker').slideUp();
        }
    });
    canvas_f.observe('object:selected', canvasEvents.objectSelected);
    canvas_f.observe('before:selection:cleared', canvasEvents.objectUnselected);
    canvas_b.observe('object:selected', canvasEvents.objectSelected);
    canvas_b.observe('before:selection:cleared', canvasEvents.objectUnselected);
    canvas_l.observe('object:selected', canvasEvents.objectSelected);
    canvas_l.observe('before:selection:cleared', canvasEvents.objectUnselected);
    canvas_r.observe('object:selected', canvasEvents.objectSelected);
    canvas_r.observe('before:selection:cleared', canvasEvents.objectUnselected);
